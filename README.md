...
- 已接受、尚未实现：Git-to-Deploy、Docker Runtime Inventory direct/Agent 持续 Event 游标订阅与权限查询、Agent 自动安装与证书轮换、Template、多主机系统验收和安全 Terminal
...
Docker Runtime Inventory 已建立独立领域模型、MongoDB observation/history/current Repository、v12-v14 索引、direct/Agent 可复用的四类 Docker List 安全 mapper、精确字节有界分块，以及 Agent `prepare → pull chunk → release` 内存快照协议与两种模式的持久化编排。新 generation 在全部分块完成前不会修改 current presence；完整提交会在同一事务标记 absent、恢复 present 并切换 head。Event 安全摘要只能提前全量对账，不能直接删除资源。direct 与 Agent 都会读取真实 Engine snapshot window Event，避免资源恰好在四类 List 之间变化后长期留下混合视图。未完成批次两小时后回收，延迟旧 observation 不能覆盖新视图；模型不包含原始 Inspect、Environment 值、Registry authorization、Volume options/status 或宿主 Mount source。Runtime Target credential/source、默认关闭的周期调度和 Event 提示调度已接线；direct/Agent 持续 Event 游标订阅、公开查询 API 和 Host/Project 权限仍未实现，详见 [docs/runtime-inventory.md](docs/runtime-inventory.md)。
