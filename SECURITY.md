...
- Webhook signing secrets and Terminal connection tickets.
...
The reusable Docker reader, safe mapper, direct persistence flow and Agent prepare/pull/release transport are implemented. Agent snapshots are memory-only, bounded to two snapshots and 32 MiB each, expire after ten minutes, and bypass both durable Agent results and the Server completed-result cache. Runtime Target source/credential wiring, default-off full/Event scheduling and authorized Project/Host queries are implemented. Real two-host disconnects, MongoDB primary changes, customer-equivalent resource peaks and browser end-to-end behavior still require system evidence. No caller should treat these components as permission to persist arbitrary Docker JSON. See [Docker Runtime Inventory](docs/runtime-inventory.md).
