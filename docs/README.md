...
| 已接受、待完成或待验收 | Evidence Worker 的生产出口系统验收和其他 KMS 客户矩阵；Web/官网/客户文档双语发布与正式客户 CLI；Git/Registry 品牌与客户网络兼容矩阵、Docker Inventory 双主机/客户等价 MongoDB 认证 TLS 故障切换/资源峰值/Web E2E、首个受保护 Agent Tag 的公开发行证据、enrollment/证书轮换真实故障注入、多主机升级/回滚系统验收、Terminal 真实远程 Linux/SSH 故障与浏览器 E2E |
