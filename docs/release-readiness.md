...
正式 Release 发布后，发布工作流会显式 dispatch `community-release-compatibility` 和 Agent 兼容矩阵；不能依赖 `GITHUB_TOKEN` 创建的 Release 事件再次触发工作流。社区矩阵会自动选择上一条已发布的 SemVer Release。首个版本只建立基线；从第二个版本开始，它会下载两版社区安装包、容器清单及 Sigstore bundle，离线验证两版清单身份和校验和，再验证两个 Server image digest 的签名并执行：
