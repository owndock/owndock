...
- 全仓竞态测试；
- MongoDB 8.3.7 单节点 Replica Set 的完整产品持久化集成、最小权限应用身份认证、`requireTLS` 加密与明文/错误 CA 拒绝，以及三成员 Replica Set 在已打开 Runtime Inventory observation 时停止 Primary、重新选举、恢复 majority 事务并原子发布 current view；
