...
func NewLoop(runner *Runner, pollInterval, operationTimeout time.Duration, onError ErrorHandler) (*Loop, error) {
	if runner == nil || pollInterval <= 0 || operationTimeout <= 0 {
		return nil, errors.New("build worker loop configuration is invalid")
	}
	return &Loop{runner: runner, pollInterval: pollInterval, operationTimeout: operationTimeout, onError: onError}, nil
}
