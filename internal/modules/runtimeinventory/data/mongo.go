...
func (r *MongoRepository) Current(
	...
	if err != nil {
		return nil, fmt.Errorf("find current runtime inventory resources: %w", err)
	}
	defer func() { _ = cursor.Close(ctx) }()
	var documents []resourceDocument
	if err := cursor.All(ctx, &documents); err != nil {
		return nil, fmt.Errorf("decode current runtime inventory resources: %w", err)
	}
	resources := make([]biz.Resource, len(documents))
	for index, document := range documents {
		resources[index] = document.domain()
	}
